...
class AMF0DecodingTests: XCTestCase {
    ...
    func testAMFObject() {
        ...
        let value = try? decoder.decode(AMFCommand.self, from: data)
        XCTAssertEqual(value?.commandName, "createStream")
        XCTAssertEqual(value?.transactionID, 2)
    }
    ...
    func testLargerAMFObject() {
        ...
        let value = try? decoder.decode(AMFCommand.self, from: data)
        XCTAssertEqual(value?.commandName, "_result")
        XCTAssertEqual(value?.transactionID, 1)
        XCTAssertNotNil(value?.commandObject)
        XCTAssertEqual(value?.commandObject?.fmsVer, "FMS/3,5,5,2004")
        XCTAssertEqual(value?.commandObject?.capabilities, 31)
        XCTAssertEqual(value?.commandObject?.mode, 1)
    }

    func testEndAMFObject() {

        let data = Data(bytes: [])
        let value = try? decoder.decode(AMFLevel.self, from: data)
